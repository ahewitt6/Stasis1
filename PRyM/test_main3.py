...
# ! ######################################################################################################################################################################################################################################
    ...
    def _rad_and_dm_blocks(self, Tg, Tnue, Tnumu, p):
        ...
        rho_rad_end = (PRyMthermo.rho_g(T_end_stasis) + PRyMthermo.rho_e(T_end_stasis) + 3. * PRyMthermo.rho_nu(T_end_stasis))
        rho_tot_end = rho_rad_end
        H_end = np.sqrt(8*np.pi*PRyMini.GN/3 * rho_tot_end)
