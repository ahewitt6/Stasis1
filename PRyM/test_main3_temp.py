...
class PRyMclass(object):
    def __init__(self,my_rho_NP=0.,my_p_NP=0.,my_drho_NP_dT=0.,my_delta_rho_NP=0., stasis_params=None):
        ...
        #T_end_temp = self.stasis_params['T_end']      #test
        #Tend_MeV = self.stasis_params['T_end']
        #print(self.Hubble(T_end_temp, T_end_temp*0, T_end_temp*0),self.Hubble(Tstart_MeV, Tstart_MeV*0, Tstart_MeV*0))
        #print(self.stasis_params['N_SPECIES']*self.stasis_params['Delta_m'])
        #print(self.stasis_params['gamma'])
        #self.stasis_params['GAMMA_OVER_H'] = self.Hubble(T_end_temp, T_end_temp, T_end_temp)*(self.stasis_params['N_SPECIES']*self.stasis_params['Delta_m']/self.stasis_params['m0'])**(self.stasis_params['gamma'])/(self.Hubble(Tstart_MeV, Tstart_MeV, Tstart_MeV))    #test
...
# ! ######################################################################################################################################################################################################################################
    ...
    #test, trying to modify it so we can choose Tend
    def _rad_and_dm_blocks(self, Tg, Tnue, Tnumu, p):
        m_ell = p["m_ell"]
        m0    = p["m0"]
        alpha = p["alpha"]
        ...
        Gamma0_mev = self.Hubble(T_end, Tnue, Tnumu)
        Gamma_ell_mev  = Gamma0_mev * (m_ell/m0)**gamma
        #print(H_mev)
        #print(Gamma0_mev)
