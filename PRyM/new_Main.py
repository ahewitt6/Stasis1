...
class PRyMclass(object):
    def __init__(self,my_rho_NP=0.,my_p_NP=0.,my_drho_NP_dT=0.,my_delta_rho_NP=0., stasis_params=None):
        ...
        if PRyMini.stasis_flag:
            # convert Kelvin inputs to MeV
            self.Stasis_start_MeV = PRyMini.stasis_params["T_start_K"] / PRyMini.MeV_to_Kelvin
            self.Stasis_end_MeV = PRyMini.stasis_params["T_end_K"]   / PRyMini.MeV_to_Kelvin
            # matter:radiation ratio and effective w
            self.Omega_M_stasis = PRyMini.stasis_params["Omega_M"]
            self.kappa = PRyMini.stasis_params["Omega_M"] / (1.0 - PRyMini.stasis_params["Omega_M"])
            # self.w_eff = 1.0 / (3.0 * (1.0 + self.kappa))
            self.w_eff = (1 - self.Omega_M_stasis)/3
        ...
        #     # This should compute the total energy density from the relevant species
        ...
        if(PRyMini.compute_bckg_flag):
            ...
            if(PRyMini.NP_thermo_flag):
                ...
                if(PRyMini.julia_flag):
                    ...
                    # define stasis‐exit event
                    # def event_exit_stasis(t, y):
                    #     return y[0] - self.Stasis_end_MeV
                    
                    # event_exit_stasis.terminal = True
                    # event_exit_stasis.direction = -1

                    # sol1 = solve_ivp(
                    #     fun=self.dTtotdt,
                    #     t_span=(tini, tfin),
                    #     y0=Tini_vec.copy(),
                    #     events=event_exit_stasis,
                    #     method='Radau',     # stiff integrator
                    #     rtol=1e-6,
                    #     atol=1e-9,
                    # )

                    # # Did we ever trigger stasis exit?
                    # if sol1.t_events[0].size == 0:
                    #     # no stasis window hit → fall back to the original single solve
                    ...
                    #     )
                    # else:
                    #     # We did hit the end of stasis
                    #     t_exit = sol1.t_events[0][0]
                    #     y_exit = sol1.y[:, -1].copy()
                    #     # enforce exact plateau‐end temperature
                    #     y_exit[0] = self.Stasis_end_MeV

                    #     # Carve up eval grid
                    #     mask_before  = sol_thermo_sampling <= t_exit
                    #     mask_after   = sol_thermo_sampling  > t_exit
                    #     t_before     = sol_thermo_sampling[mask_before]
                    #     t_after      = sol_thermo_sampling[mask_after]

                    #     # Build the “before” piece from sol1
                    #     #    (only uses sol1.t to find matching y’s)
                    #     idxs = np.searchsorted(sol1.t, t_before)
                    #     y_before = sol1.y[:, idxs]

                    # # Build the flat plateau for any points lying exactly at or below t_exit
                    # t_plateau = t_before[-1:][0:0]  # empty placeholder
                    # if mask_before.sum() < sol_thermo_sampling.size:
                    #     # points strictly between t_before[-1] and t_exit
                    #     plateau_mask = (sol_thermo_sampling > t_before[-1]) & (sol_thermo_sampling <= t_exit)
                    #     t_plateau    = sol_thermo_sampling[plateau_mask]
                    #     # photon T stays == Stasis_end; other species held at their exit value
                    #     y_plateau    = np.vstack([
                    #         np.full_like(t_plateau, self.Stasis_end_MeV),
                    #         *(np.full_like(t_plateau, y_exit[i]) for i in range(1, len(y_exit)))
                    #     ])
                    # else:
                    #     y_plateau = np.empty((len(y_exit), 0))

                    # # resume integration past t_exit
                    # sol2 = solve_ivp(
                    #     fun=self.dTtotdt,
                    #     t_span=(t_exit, tfin),
                    #     y0=y_exit,
                    ...
                    # t_full = np.hstack((t_before, t_plateau, sol2.t))
                    # y_full = np.hstack((y_before, y_plateau, sol2.y))

                    # # original sol_thermo object
                    # sol_thermo = types.SimpleNamespace(t=t_full, y=y_full)
            ...
            if(PRyMini.save_bckg_flag):
                ...
                rho_gamma = PRyMthermo.rho_g(self.Tg_vec)
                # neutrino energy density per flavor
                rho_nu    = PRyMthermo.rho_nu(self.Tnu_vec)
                rho_rad   = rho_gamma + 3.0*rho_nu

                # This is erroring out. ValueError: The truth value of an array with more than one element is ambiguous. Use a.any() or a.all()
                # Dont compute back flag for now
                rho_matter = PRyMthermo.rho_e(self.Tg_vec)

                mask = ( (self.Tg_vec < self.Stasis_start_MeV) & (self.Tg_vec > self.Stasis_end_MeV) )

                os.makedirs(my_dir + "/PRyMrates/thermo", exist_ok=True)
                np.savetxt(
                    my_dir + "/PRyMrates/thermo/rho_matter_radiation.txt",
                    np.c_[self.t_vec, rho_matter, rho_rad],
                    header="t[s]    rho_matter    rho_radiation",
                    fmt="%.6e"
                )
...
# ! ######################################################################################################################################################################################################################################
    ...
    def Hubble(self, Tg,Tnue,Tnumu,T_NP=0.):
        ...
        # STASIS branch: only if within (T_end, T_start)
        # I am trying to implement stasis. This is a theory that produces a tower of particles, blahblahblah. 
        # Basically it will set a period of time where Omega_M is equal to some certain amount for an extended period of time. 
        # This overall changes the w_eff, etc. I think I am tracking that so far. 
        # However, in the code nothing is changing and there is not a change in the abundances of matter and radiation. 
        # If during stasis Omega_M =0.6, then Omega_gamma = 0.4 and the densities should reflect that. How can I make this impelmented into the actual code though?
        if PRyMini.stasis_flag:
            if self.Stasis_start_MeV > Tg > self.Stasis_end_MeV:
                # print("RING RING RING")
                rho_R = rho_tot
                rho_M = self.kappa * rho_R
                rho_tot = rho_R + rho_M
                return PRyMini.MeV_to_secm1 * np.sqrt(8 * np.pi * PRyMini.GN / 3 * rho_tot)
    ...
    def dTnudt(self,Tg,Tnue,Tnumu,T_NP=0.):
        ...
        # If we want to say that Stasis treats neutrinos differently for some reason???
        if PRyMini.stasis_flag and (self.Stasis_start_MeV > Tg > self.Stasis_end_MeV):
            # rho_tot = 3 * H*H / (8 * np.pi * PRyMini.G)

            # rho_rad = (1-self.Omega_M_stasis) * rho_tot

            # # raw radiation for the ratio f_nu
            # rho_rad_raw = ( PRyMthermo.rho_nu(Tnue) + PRyMthermo.rho_g(Tg) + PRyMthermo.rho_e(Tg) )
            # f_nu = PRyMthermo.rho_nu(Tnue) / rho_rad_raw

            # # stasis neutrino density & its dρ/dT
            # rho_nu = f_nu * rho_rad
            # drho_nu_dT = 3.0 * (1.0 + self.w_eff) * rho_nu / Tnue

            # num = 4 * H * rho_nu # No w_eff because neutrinos are always relativistic here

            # # num   = 3.0 * H * PRyMthermo.rho_nu(Tnue)
            # delta = PRyMthermo.delta_rho_nue(Tg, Tnue, Tnumu) + 2.0 * PRyMthermo.delta_rho_numu(Tg, Tnue, Tnumu)
            # if PRyMini.NP_thermo_flag:
            #     delta += PRyMthermo.delta_rho_NP(Tg, Tnue, Tnumu, T_NP)
            # num += delta
            # # den = 3.0 * PRyMthermo.drho_nu_dT(Tnue)
            # den = drho_nu_dT
            # if PRyMini.NP_nu_flag:
            #     num += 3.0 * H * (PRyMthermo.rho_NP(Tnue) + PRyMthermo.p_NP(Tnue))
            #     den += PRyMthermo.drho_NP_dT(Tnue)
            # # return -(4 - self.Omega_M_stasis) * num / den
            # return 

            # Neutrinos have their own w_nu and fluid equation, I dont know if it actually does get affected by stasis
            # There might not even be a need to call this at all since neutrinos are part of the plasma
    ...
    def dTgdt(self,Tg,Tnue,Tnumu,T_NP=0.):
        ...
        # --- STASIS temperature slope for plasma ---
        if PRyMini.stasis_flag and (self.Stasis_start_MeV > Tg > self.Stasis_end_MeV):
            # num   = H * ( PRyMthermo.rho_g(Tg) + PRyMthermo.rho_e(Tg) + PRyMthermo.p_e(Tg) + Tg * PRyMthermo.dPdT(Tg) )
            # total density from Friedmann
            rho_tot = 3 * H*H / (8 * np.pi * PRyMini.GN)

            num = (1 + self.w_eff) * H * rho_tot
            delta = PRyMthermo.delta_rho_nue(Tg, Tnue, Tnumu) + 2.0 * PRyMthermo.delta_rho_numu(Tg, Tnue, Tnumu)

            # Lets ignore NP contribtions for now
            if PRyMini.NP_thermo_flag:
                delta += PRyMthermo.delta_rho_NP(Tg, Tnue, Tnumu, T_NP)

            num += delta

            # denominator via the analytic derivative + Second order contribution
            den = 3.0 * (1.0 + self.w_eff) * rho_tot / Tg + Tg * PRyMthermo.d2PdT2(Tg)
            # den  = PRyMthermo.drho_g_dT(Tg) + PRyMthermo.drho_e_dT(Tg) + Tg * PRyMthermo.d2PdT2(Tg)

            # Ignoring NP in stasis for now
            if PRyMini.NP_e_flag:
                num += 3.0 * H * (PRyMthermo.rho_NP(Tg) + PRyMthermo.p_NP(Tg))
    ...
    # # Weak rates at HT
    # def nTOp_frwrd(self,T):
    #     return self.NormWeakRates*self.nTOp_frwrd_HT(T)
    # def nTOp_bkwrd(self,T):
    #     return self.NormWeakRates*self.nTOp_bkwrd_HT(T)
        
    # def Yn_prime_HT(self,t,Y):
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     return self.nTOp_bkwrd(T_t)*Y[1]-self.nTOp_frwrd(T_t)*Y[0]
        
    # def Yp_prime_HT(self,t,Y):
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     return self.nTOp_frwrd(T_t)*Y[0]-self.nTOp_bkwrd(T_t)*Y[1]
        
    # def Y_prime_HT(self,t,Y):
    #     dY = self.Yn_prime_HT(t,Y),self.Yp_prime_HT(t,Y)
    #     return dY
    ...
    #     rhoBBN = self.rhoB_BBN(self.a_of_t(t))
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     dY = PRyMnucl.dYndt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYpdt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYddt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYtdt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYHe3dt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYadt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi7dt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYBe7dt(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd)
    #     return dY
    ...
    #     rhoBBN = self.rhoB_BBN(a_of_t(t))
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     dY = PRyMnucl.dYndtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYpdtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYddtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYtdtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYHe3dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYadtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi7dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYBe7dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYHe6dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi8dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi6dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYB8dtMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd)
    #     return dY
        
    # def Jacobian_MT(t,Y):
    #     rhoBBN = self.rhoB_BBN(a_of_t(t))
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     return PRyMnucl.JacobianMT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd)

    # def Y_prime_LT(t,Y):
    #     rhoBBN = self.rhoB_BBN(a_of_t(t))
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     dY = PRyMnucl.dYndtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYpdtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYddtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYtdtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYHe3dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYadtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi7dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYBe7dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYHe6dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi8dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYLi6dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd),PRyMnucl.dYB8dtLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd)
    #     return dY

    # def Jacobian_LT(t,Y):
    #     rhoBBN = self.rhoB_BBN(a_of_t(t))
    #     T_t = self.T_of_t(t)*PRyMini.MeV_to_Kelvin # temperature in [K]
    #     return PRyMnucl.JacobianLT(Y,T_t,rhoBBN,self.nTOp_frwrd,self.nTOp_bkwrd)
