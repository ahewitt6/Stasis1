    ...
    def __init__(self,my_rho_NP=0.,my_p_NP=0.,my_drho_NP_dT=0.,my_delta_rho_NP=0., stasis_params=None):
        ...
        if(PRyMini.compute_bckg_flag):
            ...
            if(PRyMini.save_bckg_flag):
                ...
                #rho_rad = PRyMthermo.rho_g(self.Tg_vec)     #test
                #H_vec = np.sqrt((np.array(rho_dm)+np.array(rho_rad))*8*np.pi*PRyMini.GN/3)/(6.582_119_569e-22)  #test
                ...
                print('Omega_M0: ',Omega_M,' Omega_rad0: ',Omega_rad)
                ...
                out = np.column_stack([ self.t_vec, self.Tg_vec, rho_tot, rho_rad, rho_dm, Omega_M, Omega_rad,H_vec])
                # header = "t_s [s], Tg [MeV], rho_tot, rho_rad, rho_m, rho_dm, rho_m_check, Omega_m, Omega_rad, Omega_m_check"
                header = "t_s [s], Tg [MeV], rho_tot, rho_rad, rho_dm, Omega_m, Omega_rad,H"
...
# ! ######################################################################################################################################################################################################################################
    ...
    def _rad_and_dm_blocks(self, Tg, Tnue, Tnumu, p):
        """
        Everything that depends on the current temperature,
        plus any static keys we need for DM tower.
        """
        m_ell = p["m_ell"]
        m0    = p["m0"]
        print('m0: ',m0)
        alpha = p["alpha"]
        gamma = p["gamma"]
        G_H   = p["GAMMA_OVER_H"]  # whatever you called it in stasis_params
        tiny_radiation = p["tiny_radiation"]

        # --- dynamic, T-dependent pieces ---
        rho_rad = (
            PRyMthermo.rho_g(Tg) + PRyMthermo.rho_e(Tg)
          - PRyMthermo.PofT(Tg) + Tg * PRyMthermo.dPdT(Tg)
          + PRyMthermo.rho_nu(Tnue) + 2. * PRyMthermo.rho_nu(Tnumu)
        )

        # Tstart_MeV = PRyMini.T_start/PRyMini.MeV_to_Kelvin
        # rho_rad0 = (
        #     PRyMthermo.rho_g(Tstart_MeV)  + PRyMthermo.rho_e(Tstart_MeV)
        #   - PRyMthermo.PofT(Tstart_MeV) + Tstart_MeV * PRyMthermo.dPdT(Tstart_MeV)
        #   + PRyMthermo.rho_nu(Tstart_MeV) + 2. * PRyMthermo.rho_nu(Tstart_MeV)
        # )
        Tstart_MeV = PRyMini.T_start/PRyMini.MeV_to_Kelvin

        # 2) sum only true energy densities, not enthalpy pieces
        rho_rad0 = (
            PRyMthermo.rho_g(Tstart_MeV)
        + PRyMthermo.rho_e(Tstart_MeV)
        + 2. * PRyMthermo.rho_nu(Tstart_MeV)
        )
        rho_gamma0 = rho_rad0
        #rho_gamma0 =  29606.63436125509     #test rho_M0:  2931056.801764254  rho_tot0:  2960663.436125509
        ...
        print('rho_gamma0: ', rho_gamma0, ' rho_M0: ',rho_M0,' rho_tot0: ',rho_tot0)

        out = {
            "H":            H_mev,
            "H0_s":         self.mev_to_inv_s(H_mev),
            "rho_rad":      rho_rad0,
        ...
        omega = (m_ell/m0)**alpha #0.99 is a test
        omega /= (omega.sum())
        #omega *= (1-tiny_radiation)    #test
        print('omega0: ',omega)
        rho_ell0 = omega * rho_M0
        print('rho_ell0: ',rho_ell0)
        ...
        out["Gamma_ell_s"] = self.mev_to_inv_s(Gamma_ell_mev)

        return out


    def mev_to_inv_s(self,x_mev):
    ...
    def Hubble(self, Tg,Tnue,Tnumu,T_NP=0.):
        ...
        # rho_inject = getattr(self, "_rho_rad_inject", 0.0)
        rho_inject = 0.0
    ...
    def dTnudt(self,Tg,Tnue,Tnumu,T_NP=0.):
        ...
        # num += Q_stasis_nu
        # num += getattr(self,"_Qdot",0.0) * (3/3)
    ...
    def dTgdt(self,Tg,Tnue,Tnumu,T_NP=0.):
        ...
        # den += self.dQ_dT(num, Tg)

        # Make parameter for fraction going into neutrinos / gamma called f
        
        # Model of stasis for now is only injecting into photons. Always adding to plasma bath <==> injecting into photons
    ...
    # Move tower back before PRyMordial starts
    ...
    def rhs(self, t, y):
        # nT = 3 if PRyMini.NP_thermo_flag else 2               # # of temps
        nT = 2
        T = y[:nT]                                          # MeV
        off = nT
        N = self.stasis["N_SPECIES"]

        chi_ell = y[off : off+N]                              # (N,)
        chi_g   = y[off+N]
        # Nefolds = y[off+N+1]

        # ---------- energy densities in MeV⁴ ----------
        rho_ell = np.exp(chi_ell)
        rho_dm  = rho_ell.sum()
        rho_g   = np.exp(chi_g)
        Gamma = self.stasis["Gamma_ell_s"]  # s⁻¹
        self._Qdot = np.sum(Gamma * rho_ell)
        # self._Qdot = np.sum(Gamma * np.exp(chi_ell - chi_g))

        self._rho_dm_current = rho_dm
        # self._rho_rad_inject = rho_g

        # thermal-plasma radiation
        Tg, Tnu = T[0], T[1]
        rho_rad = (
            PRyMthermo.rho_g(Tg) + PRyMthermo.rho_e(Tg)
        - PRyMthermo.PofT(Tg) + Tg*PRyMthermo.dPdT(Tg)
        + PRyMthermo.rho_nu(Tnu) + 2.*PRyMthermo.rho_nu(Tnu)
        )
        # rho_rad = rho_g

        # expansion rate (s⁻¹)
        H = self.Hubble(Tg, Tnu, Tnu)
        #rho_rad = PRyMthermo.rho_g(Tg)  #test
        #H = np.sqrt((np.array(rho_dm)+np.array(rho_rad))*8*np.pi*PRyMini.GN/3)/(6.582_119_569e-22)  #test
    ...
    def a_of_T(self,T):
        ...
        # * Newly I think I need to evolve according to the w_eff during stasis here, densities also get changed but get transfer functions
        p = self.stasis_params
