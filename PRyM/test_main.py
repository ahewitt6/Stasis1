...
class PRyMclass(object):
    def __init__(self,my_rho_NP=0.,my_p_NP=0.,my_drho_NP_dT=0.,my_delta_rho_NP=0., stasis_params=None):
        ...
        self.stasis_params = dict(
            ...
            dyn_T_end = PRyMini.stasis_params["T_end_K"] / PRyMini.MeV_to_Kelvin,
        )
        ...
        if(PRyMini.compute_bckg_flag):
            ...
            if(PRyMini.save_bckg_flag):
                ...
                out = np.column_stack([ self.t_vec, self.Tg_vec, rho_tot, rho_rad, rho_m, rho_dm, rho_m_check, Omega_m, Omega_rad, Omega_m_check])
...
# ! ######################################################################################################################################################################################################################################
    ...
    def _stasis_on(self, T):
        p = self.stasis_params
        return p["enabled"] and (p["stasis_end_MeV"] < T < p["stasis_start_MeV"])
    ...
    def Hubble(self, Tg,Tnue,Tnumu,T_NP=0.):
        ...
        p = self.stasis_params
        # if p["enabled"] and (p["stasis_end_MeV"] < Tg < p["stasis_start_MeV"]):
        #     rho_m_eff = self.stasis_params["kappa"]*rho_rad
        #     rho_tot = rho_rad + rho_m_eff

        # else:
        #     rho_tot = rho_rad

        if (not p["enabled"]) or (Tg > p["stasis_start_MeV"]):
            rho_tot = rho_rad
            return PRyMini.MeV_to_secm1 * np.sqrt(8*np.pi*PRyMini.GN/3 * rho_tot)
        

        # Code tower decaying into rad continuionously over stasis window. 
        # Tower states: N. Tower rung decaying at Twindow/N temp during window
        # Tower rung decays: rho_m_rung = rho_m0 * height -> rho_m0 + (rho_g + rho_e+- + rho_nu)
        # Should keep rho_tot in equilibrium as expected.  

        rho_dm_stasis = 0.0
        if p["enabled"]:
            # during stasis window inject kappa·ρ_rad
            if p["stasis_end_MeV"] < Tg < p["stasis_start_MeV"]:
                rho_dm_stasis = p["kappa"] * rho_rad

            else:
                # only once when exiting stasis _> record the freeze‐in
                if Tg <= p["stasis_end_MeV"] and p["rho_m0"] == 0:
                    p["rho_m0"] = p["kappa"] * rho_rad # Remember rho_M <= rho_DM. Omega_DM h^2 = 0.112
                    p["dyn_T_end"] = Tg

                # after that -> red‐shift like matter goes like a^-3 ~ (T/T_end)^3
                if p["rho_m0"] is not None:
                    rho_dm_stasis = p["rho_m0"] * (Tg / p["dyn_T_end"])**3

        rho_tot = rho_rad + rho_dm_stasis
