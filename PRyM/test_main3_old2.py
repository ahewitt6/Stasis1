...
class PRyMclass(object):
    def __init__(self,my_rho_NP=0.,my_p_NP=0.,my_drho_NP_dT=0.,my_delta_rho_NP=0., stasis_params=None):
        ...
        self.stasis_params = dict(
            ...
            kappa = PRyMini.stasis_params["Omega_M"] / (1 - PRyMini.stasis_params["Omega_M"])
            # kappa = PRyMini.stasis_params["kappa"] # ρ_tower / ρ_rad
        )
        ...
        if(PRyMini.compute_bckg_flag):
            ...
            if(PRyMini.save_bckg_flag):
                ...
                out = np.column_stack([ self.t_vec, self.Tg_vec, rho_tot, rho_rad, rho_m, rho_m_check, Omega_m, Omega_rad, Omega_m_check])
                header = "t_s [s], Tg [MeV], rho_tot, rho_rad, rho_m, rho_m_check, Omega_m, Omega_rad, Omega_m_check"
...
# ! ######################################################################################################################################################################################################################################
    ...
    def Hubble(self, Tg,Tnue,Tnumu,T_NP=0.):
        ...
        if p["enabled"] and (p["stasis_end_MeV"] < Tg < p["stasis_start_MeV"]):
            rho_rad = rho_tot.copy()
            rho_tot += self.stasis_params["kappa"] * rho_rad
